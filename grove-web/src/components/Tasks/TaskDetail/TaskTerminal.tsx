...
export function TaskTerminal({ task, onStartSession }: TaskTerminalProps) {
  ...
  if (!isLive) {
    return (
      <div className="mx-4 my-3 rounded-lg border border-[var(--color-border)] bg-[#0d0d0d] overflow-hidden">
  ...
  return (
    <motion.div
      ...
      className="mx-4 my-3 rounded-lg border border-[var(--color-border)] bg-[#0d0d0d] overflow-hidden"
    >
      {/* Terminal Header */}
      <div className="flex items-center justify-between px-3 py-2 bg-[var(--color-bg)] border-b border-[var(--color-border)]">
        <div className="flex items-center gap-2 text-sm text-[var(--color-text-muted)]">
      ...
      <div
        ref={containerRef}
        className="h-[180px] p-3 overflow-y-auto font-mono text-sm leading-relaxed"
