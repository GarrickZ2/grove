...
/// 渲染项目详情面板
pub fn render(frame: &mut Frame, area: Rect, detail: &ProjectDetail, colors: &ThemeColors) {
    ...
    if !detail.active_tasks.is_empty() {
        ...
        for task in &detail.active_tasks {
            lines.push(task_line(task.name.as_str(), task.status, task.additions, task.deletions, colors));
        }
        lines.push(Line::from(""));
    }
...
/// 任务行
fn task_line(name: &str, status: WorktreeStatus, additions: u32, deletions: u32, colors: &ThemeColors) -> Line<'static> {
